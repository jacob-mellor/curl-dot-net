...
`)})}function Co(c){localStorage.setItem("theme",c),c==="auto"?document.documentElement.setAttribute("data-bs-theme",window.matchMedia("(prefers-color-scheme: dark)").matches?"dark":"light"):document.documentElement.setAttribute("data-bs-theme",c)}async function Xo(){return localStorage.getItem("theme")||(await D()).defaultTheme||"auto"}async function Zo(){Co(await Xo())}function Bo(c){return new MutationObserver(()=>c(Rb())).observe(document.documentElement,{attributes:!0,attributeFilter:["data-bs-theme"]})}function Rb(){return document.documentElement.getAttribute("data-bs-theme")}async function Wo(c){let t=await Xo(),i=t==="light"?"sun":t==="dark"?"moon":"circle-half";return u`
    <div class='dropdown'>
      ...
      <ul class='dropdown-menu dropdown-menu-end'>
        ...
        <nav>
          <ul class="pagination">
            <li class="page-item">
              <a class="page-link ${Ut({disabled:n<=0})}" href="#" aria-label="Previous"
                @click="${()=>B(n-1)}">
            ...
            ${G.map(Z=>u`
              <li class="page-item">
                <a class="page-link ${Ut({active:n===Z})}" href="#"
                  @click="${()=>B(Z)}">${Z+1}</a></li>`)}
            <li class="page-item">
              <a class="page-link ${Ut({disabled:n>=s-1})}" href="#" aria-label="Next"
                @click="${()=>B(n+1)}">
                <span aria-hidden="true">&raquo;</span>
              </a>
            </li>
          </ul>
        </nav>`;function B(Z){Z>=0&&Z<s&&I(a,Z)}}}}function Vb(c,t){let b=t.split(/\s+/g).map(I=>I.toLowerCase()),e=c.toLowerCase(),o=[],l=0;for(let I=0;I<b.length;I++){let a=b[I],n=e.indexOf(a,l);n>=0&&(o.push(u`${c.slice(l,n)}`),o.push(u`<b>${c.slice(n,n+a.length)}</b>`),l=n+a.length)}return o.push(u`${c.slice(l)}`),u`${o}`}async function Qo(){let c=It("docfx:tocrel");if(!c)return[];let t=It("docfx:disabletocfilter")==="true",i=new URL(c.replace(/.html$/gi,".json"),window.location.href),{items:b,pdf:e,pdfFileName:o}=await(await fetch(i)).json(),l=t?"":localStorage?.getItem("tocFilterUrl")||"",I=t?"":localStorage?.getItem("tocFilter")||"";(l===""||l!==i.toString())&&(I="",t||(localStorage?.setItem("tocFilter",""),localStorage?.setItem("tocFilterUrl",i.toString())));let a=[],n=[];b.forEach(s);let r=document.getElementById("toc");if(r){X();let G=r.querySelectorAll("li.active"),B=G[G.length-1];B&&(B.scrollIntoView({block:"nearest"}),location.hash&&(location.href=location.href))}return n.length>0&&Jn(b,n[0]),a.slice(0,-1);function s(G){let B;if(G.href){let Z=new URL(G.href,i);G.href=Z.href,B=pc(Z)?!1:Go(Z,window.location),B&&(G.items&&(G.expanded=!0),n.push(G))}if(G.items)for(let Z of G.items)s(Z)&&(B=!0,G.expanded=!0);return B?(a.unshift(G),!0):!1}function X(){lt(u`
      ${F()}
      <div class="flex-fill overflow-y-auto">${g(b)||d()}</div>
      ${C()}`,r)}function g(G){let B=G.map(Z=>{let{href:p,name:x,items:Q,expanded:R}=Z;if(!x)return null;let U=!Q||Q.length<=0,A=U?null:g(Q);if(I!==""&&!A&&!x.toLowerCase().includes(I.toLowerCase()))return null;let y=p?u`<a class='${Ut({"nav-link":!a.includes(Z)})}' href=${p}>${ct(x)}</a>`:U?u`<span class='text-body-tertiary name-only'>${ct(x)}</a>`:u`<a class='${Ut({"nav-link":!a.includes(Z)})}' href='#' @click=${T}>${ct(x)}</a>`,v=I!==""&&R!==!1&&A!=null||R===!0;return u`
        <li class=${Ut({expander:!U,expanded:v,active:a.includes(Z)})}>
          ${U?null:u`<span class='expand-stub' @click=${T}></span>`}
          ${y}
          ${A}
      ...
      <form class='filter'>
        ...
        <input class='form-control' @input=${G} value='${I}' type='search' placeholder='${L("tocFilter")}' autocomplete='off' aria-label='${L("tocFilter")}'>
      </form>`;function G(B){I=B.target.value.trim(),localStorage?.setItem("tocFilter",I),X()}}function d(){return I===""?null:u`<div class='no-result'>${L("searchNoResults",{query:I})}</div>`}function C(){return e?u`<div class="py-2 mb-md-4"><a class="pdf-link" href="${new URL(o||"toc.pdf",i)}">${L("downloadPdf")}</a></div>`:null}}function Jn(c,t){let i=document.getElementById("nextArticle");if(!i)return;let b=n(c),e=b.findIndex(r=>r===t),o=b[e-1],l=b[e+1];if(!o&&!l)return;let I=o?u`<div class="prev"><span><i class='bi bi-chevron-left'></i> ${L("prevArticle")}</span> <a href="${o.href}" rel="prev">${ct(o.name)}</a></div>`:null,a=l?u`<div class="next"><span>${L("nextArticle")} <i class='bi bi-chevron-right'></i></span> <a href="${l.href}" rel="next">${ct(l.name)}</a></div>`:null;lt(u`${I} ${a}`,i);function n(r){let s=[];for(let X of r)X.href&&s.push(X),X.items&&s.push(...n(X.items));return s}}var Uo=No(Ro(),1);async function ho(){let c=document.getElementById("navbar");if(!c)return[];let{iconLinks:t}=await D(),i=await I(),b=Nn(i),e=a=>u`<li class='nav-item'><a class='nav-link ${a===b?"active":null}' aria-current=${a===b?"page":!1} href=${a.href}>${ct(a.name)}</a></li>`,o=u`
