...
sed -i '' 's/: base($"Could not resolve host: {hostname}", hostname, null, command)$/: base($"Could not resolve host: {hostname}", 6, command)/' src/CurlDotNet/Exceptions/CurlExceptions.cs
sed -i '' '/CurlDnsException.*{/,/^    }$/ { /CurlErrorCode = 6;/d; }' src/CurlDotNet/Exceptions/CurlExceptions.cs
...
sed -i '' '/public CurlProxyException(string proxyHost, int proxyPort, string command = null)/,/^        }$/ {
    ...
    /CurlErrorCode = 5;/d
}' src/CurlDotNet/Exceptions/CurlExceptions.cs
...
sed -i '' '/public CurlFtpAcceptTimeoutException(string message, string command = null)/,/^        }$/ {
    ...
    /CurlErrorCode = 12;/d
}' src/CurlDotNet/Exceptions/CurlExceptionTypes.cs
