...
**Always append to `/claude_code_transcript.log` during every session to preserve conversation history and decisions. This ensures continuity if the session is interrupted.**
...
- **Always append to `/claude_code_transcript.log` during sessions**